...
The following video explains DNS.  

<div class='not-content'>
  <YouTube id='mpQZVYPuDGU' params='fs=1&modestbranding=1&rel=0&autoplay=1' />
</div>

:::tip[Activity: Application layer analysis in Wireshark : You can skip this activity as we are going to do a full analysis of network traffic at the end of this section]

In Wireshark:

- Capture traffic of web application
...
2. SYN-ACK: Server acknowledges and sends its own synchronisation
3. ACK: Client acknowledges the server's synchronisation
...
- You can identify which applications use TCP or UDP and you may need to capture traffic of different applications to analyse.
...
Subnetting is a concept that is used to divide IP networks into smaller subnetworks. Quick guide to subnetting:
- **CIDR Notation**: e.g., 192.168.1.0/24 (the /24 indicates 24 bits for network portion and last 8 bits are used foor host addressing)
...
:::tip[Activity: Analysing Network Traffic in Wireshark]

In this activity, we will analyse the captured traffic in Raspberry Pi.

**Materials Needed:**
- Wireshark installed on Raspberry Pi

**Step 1: Capture traffic file**

Open Wireshark and load your saved capture file:

1. Close your browser
2. Open Wireshark and start capturing traffic in WiFi network
3. Open the browser and go to "http://www.bom.gov.au/vic/forecasts/melbourne.shtml" 
4. Stop packet capture in Wireshark
5. Save the packet capture and use the capture for the analysis

**Step 2: Apply a display filter to focus on relevant traffic**

For example, use can use "http" to filter out the entries related http protocol.
...
- Can you identify the IP address of your RPi? 
- Can you identify the IP address of the server?
...
3. **Transport Layer**
   - What is the transport layer protocol used?
...
- Does the communication use any encryption? How would this change if HTTPS was used?

**Submit screenshots of your Wireshark analysis with annotations highlighting the key aspects of the communication.**
:::
...
**Step 5: Analyse characteristic operations**
...
**Take screenshot of your Wireshark analysis with annotations highlighting the key aspects of the BLE communication.**
:::
*/}
...
In the next section, we'll introduce basic concepts of network security, explaining how the communication we've been analysing can be protected from various threats.
