...
- **Data link layer**: The sorting facilities that organise mail by region
...
We are going to learn about each layer and their protocols in details in the next section. Until then, let's quickly get ourselves familiarise with high level functions that common application layer and transport layer protocols provide.

### Common Application Layer Protocols

The application layer contains many specialised protocols for different services:
...
- Specialised for file uploads and downloads
...
We use the Internet all the time to access information. How do you access all these services online or visit a website that provides a specific service? You just type the address of the webpage in the address bar of your browser. Do you know that is just the start of a lengthy process? The browser contacts the relevant web server and requests data and then assembles the webpage from the information received within few milliseconds.

:::note[Think about It]
Before you blink, you can see the webpage you requested. Do you know how many protocols involved in the process?
:::

To understand the complexity of this process, let's trace what happens when you browse a website. You might feel overwhelmed by the number of networking protocols out there—but don't worry! In this chapter, our mission is to uncover how these fascinating systems work behind the scenes to connect us to the world. That’s the real magic of networking.

Let’s start with a familiar action: opening a website. When you type a web address into your browser, you are entering what is called a URL—short for Uniform Resource Locator. This URL is not just a string of text, it is made up of different parts, each serving a specific purpose to help your device find and load the correct webpage. We will explore how this process works step by step as we go.

Let us look at an example: https://d2l.deakin.edu.au/d2l/home

- https-> the application layer protocols used
- d2l.deakin.edu.au-> Domain name (third, second, and first level) Hostname
- /d2l/home -> Different path directory, can also have a file name

When you type a website’s URL into your browser and press "Enter", your computer starts by sending a message to the web server asking to establish a connection. It then waits for a response. If the server is available, it replies to confirm that the connection can be made. Once this confirmation is received—essentially a digital “OK”—your computer proceeds to ask for a specific page from that website. This is done by sending a message known as a GET request. When the server receives this request, it responds by sending back the contents of the requested page to your device. This simple exchange of messages is how your browser retrieves the web page you asked for.

We use website names because they’re easier for people to remember than long strings of numbers. However, computers and network devices do not understand names, they rely on numerical addresses called IP addresses. That means there must be a system to translate these easy-to-remember names into numbers that machines can work with. This is where the Domain Name System (DNS) comes in.

DNS acts like the Internet’s address book. When you type a web address into your browser, the browser extracts the hostname (like "example.com") and asks the DNS service to find the matching IP address. This request is sent by the DNS client on your device to a DNS server. The server responds with the IP address tied to that hostname. Often, these IP addresses are stored temporarily in your device's memory or browser cache to speed up future visits.

Now that your browser knows the server’s IP address, it can begin establishing a connection. This involves communicating with network routers and using other protocols to find the best path to the server.

Once connected, your browser sends a request to the web server using HTTP, asking for the contents of the page you want to see. The server responds with the necessary data. Your browser then reads and assembles this data into the web page. If the server can't find the page, it returns an error message like the familiar “404 Not Found.”

Let's see this process in action now using a common tool we use daily basis, that is your web browser, well we use developer tool of the web browser.

Please watch the following video to get familiar with the Developers tool and how to inspect network activity using the tool.


<div class='not-content'>
  <YouTube id='e1gAyQuIFQo' params='fs=1&modestbranding=1&rel=0&autoplay=1' />
</div>


:::tip[Activity: Analyse Web Traffic with Browser Developer Tools]

Learn how to use browser developer tools to analyse network traffic and understand the HTTP protocol in action.
...
1. Identify the messages received/ sent between your device and other network devices to access the webpage you requested.
...
7. How many requests were needed to load the complete webpage?
