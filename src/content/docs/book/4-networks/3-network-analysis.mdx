...
The basic tools that we use to monitor the messages exchanged between executing protocols and applications are called packet sniffers. A packet sniffer is passive program and it copies (sniffs) messages being sent from and received by your device and it displays the contents of the various protocol fields of these captured messages. It does not send or receive packets itself. Instead, it receives a copy of the packets that are been transferred in your device.  In this section, we'll focus on Wireshark, which is a popular free open source packet sniffer used in the networking industry which will allow us to observe and decode network traffic, providing a window into the communication happening between devices.

## Introduction to Wireshark

Wireshark is one of the most popular network protocol analyzers in the world. It lets you capture and interactively browse the traffic running on a computer network, providing detailed information about each packet.
...
In educational settings, create controlled environments for network analysis rather than analyzing general traffic.
:::
...
In the last module, you connected RPi and Pico via Bluetooth connection and controlled the Pico LED by sending bluetooth commands. Rerun the activity and start capturing the activity on the bluetooth interface.
